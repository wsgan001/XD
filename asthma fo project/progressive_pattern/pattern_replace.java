...
public class pattern_replace
{
	...
	public pp_tree_node ROOT=new pp_tree_node();
	
	public pattern_replace(String df,String pf,String of,String ps)
	{
		PATTERN_FILENAME=pf;
		DATA_FILENAME=df;
		OUTPUT_FILENAME=of;
			...
			while((buffer=br.readLine())!=null)
			{
				...
				else
				{
					if(title.indexOf("#Large")>=0)
					{
						...
						ROOT.path_build_with_sn( (transaction_parser.sequence_score_parser(buffer.substring(0,buffer.indexOf("[")))).ITEM_SET,pattern_sn);
						pattern_sn++;
					}
				}
			}
...
//		long start_time=System.currentTimeMillis();
		try
		{
			...
			while((buffer=data_br.readLine())!=null)
			{
				...
				boolean trace_flg=ROOT.pattern_trace_for_sn_and_pattern_location( (temp_t).ITEM_SET,0,result,"");
				String temp_string=temp_t.CLASS_LABEL+":";
				if(trace_flg)
				{
					for(int i=0;i<result.size();i++)
					{
						temp_string+=result.get(i)+",";
					}
				}
...
//			System.out.println("Pattern replace attributes setting execution time:"+(System.currentTimeMillis()-start_time));
	...
	{
		//args[] {data_filename pattern_filename output_filename pattern_sn_filename} !!!!!!!
		long start_time=System.currentTimeMillis();
		if(args[0].equals("-t"))
		{
