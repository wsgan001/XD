...
{	
	...
	JCheckBox class_or_not=new JCheckBox("Considering class label",false);
	...
	private String pattern_result_filename;
	...
	public void some_action(ActionEvent ev)
	{
		try
		{
			...
			{
				if(class_or_not.isSelected())
				{
					class_sequential_pattern_miner mining_object=new class_sequential_pattern_miner();
					mining_object.ITEM_REPEAT=item_repeat.isSelected();
					if((new File(filename.getText())).isFile())
					{
						mining_object.file_mining(filename.getText(),Double.parseDouble(support.getText()));
					}
					else if((new File(filename.getText())).isDirectory())
					{
						mining_object.dir_mining(filename.getText(),Double.parseDouble(support.getText()));
					}
					pattern_result_filename=mining_object.result_filename;
					show_result(mining_object.result_filename);
				}
				else
				{
					sequential_pattern mining_object=new sequential_pattern(filename.getText(),Double.parseDouble(support.getText()));				
					mining_object.ITEM_REPEAT=item_repeat.isSelected();
					mining_object.start();
					pattern_result_filename=mining_object.result_filename;
					show_result(mining_object.result_filename);
			...
			{
				pattern_replace replacer=new pattern_replace(pattern_result_filename,filename.getText(),output_filename.getText(),pattern_sn_filename.getText());
				System.out.println(pattern_result_filename+","+filename.getText()+","+output_filename.getText()+","+pattern_sn_filename.getText());
	...
	private void show_result(String f)
	{
		try
		{
			...
			{
				buffer=br.readLine();
				if(buffer==null || buffer.indexOf("#")==0)
				{
					if(!seq_title.equals(""))
